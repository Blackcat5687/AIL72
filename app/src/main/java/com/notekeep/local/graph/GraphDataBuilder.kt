...
object GraphDataBuilder {
    ...
    fun buildPayload(
        ...
        fun addEdgeOnce(sourceId: String, targetId: String) {
            ...
            // Undirected in this app's data model today (wiki-links, tags, and labels are all
            // symmetric relations) - "none" tells the view not to draw an arrow, matching the
            // rule to only show direction when the underlying relationship actually has one.
            edges.add(GraphEdge(sourceId, targetId, "none"))
        }
        ...
        for (note in notes) {
            ...
            nodes.add(
                GraphNodeSeed(
                    ...
                    tags = allTags,
                    // All of a note's label names joined (not just the first) so a path: rule's
                    // substring match still finds any one of several categories a note belongs
                    // to - a note in both "فلسفة" and "كتب" needs path:كتب to actually match.
            ...
            // it's seen so hub nodes only exist for tags actually in use.
            for (tagName in tags) {
                val hubId = "tag_" + tagName.trim().lowercase()
                tagHubNames.putIfAbsent(hubId, tagName)
                addEdgeOnce(note.id.toString(), hubId)
            }
            // Connect this note to a hub node per assigned label/category.
            for (labelId in noteLabelIds) {
                if (labelById[labelId] == null) continue
                addEdgeOnce(note.id.toString(), "label_$labelId")
            }
        }
