 ...
 * strictly against its own field, with no bleed between them - a `tag:` query only ever looks at
 * a note's actual tag list, a `path:` (category) query only at its actual assigned categories,
 * and a plain unprefixed word only at a real note's own title, as a whole word, never inside
 * tags/categories and never against a tag or category node's own name. This keeps "the word
 * happens to appear somewhere" from being confused with "this note actually carries this tag/
 * category" - the whole point of the type prefix.
 */
object GraphQueryEngine {
    ...
    fun matches(node: GraphNode, parsed: ParsedQuery?): Boolean {
        ...
        return when (parsed.type) {
            ...
            // any single category equals the query exactly, not merely contains it.
            "path" -> node.path.isNotEmpty() && node.path.split("، ").any { it.trim().lowercase() == v }
            "file" -> false // this app has no file paths distinct from notes - kept for parity
            // Exact tag match, not substring - tag:as must not match a tag named "task".
            "tag" -> node.tags.any { it.trim().lowercase() == v }
            "line", "section" -> node.nodeKind == "note" && wholeWordMatch(node.title, v)
