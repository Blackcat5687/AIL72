...
class GraphCanvasView @JvmOverloads constructor(
    ...
    fun loadData(payload: GraphPayload, savedPositions: Map<String, Pair<Float, Float>>) {
        ...
        for (seed in payload.nodes) {
            ...
            val node = GraphNode(
                seed.id, seed.title, seed.tags, seed.path, seed.noteId, seed.isGhost, seed.nodeKind, x, y
            )
    ...
    private val hubColor = Color.parseColor("#4ADE80")
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (n in nodes) {
            if (!n.visible) continue
            val isHub = n.degree >= GraphPhysics.HUB_DEGREE_THRESHOLD
            val colors = n.colors
            ...
            if (colors.size > 1) {
                ...
                nodeFillPaint.color = colors.firstOrNull() ?: (if (isHub) hubColor else defaultNodeColor)
